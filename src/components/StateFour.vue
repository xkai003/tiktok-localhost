...
<template>
  <!-- 封号 (Status: 4) -->
  <div class="box">
        <button @click="showfour = !showfour" style="color: red;">
            封号 ({{ statefour.length }})
        </button>
        <div v-show="showfour">
            <div class="text" v-for="item in statefour" :key="item.id">
                <div class="left"><span>账号:</span><span>账号密码:</span><span>邮箱:</span><span>邮箱密码:</span><span>国家:</span><span>状态:</span></div>
                <div class="right"><span>{{ item.user }}</span><span>{{ item.password }}</span><span>{{ item.email }}</span><span>{{ item.emailPwd }}</span><span style="color: red;">{{ item.country }}</span><div class="state"><span :class="`status-${item.status}`">{{ statusMap[item.status] }}</span><select @change="changeStatus(item, $event)"><option value="" disabled selected>更改状态</option><option value="0">未登陆</option><option value="1">登陆成功</option><option value="3">登陆失败</option><option value="2">频繁登陆</option></select></div><div class="btn"><button @click="copy(item.user, $event.target)">复制账号</button><button @click="copy(item.password, $event.target)">复制账号密码</button><button @click="copy(item.email, $event.target)">复制邮箱</button><button @click="copy(item.emailPwd, $event.target)">复制邮箱密码</button></div>
                </div>
            </div>
        </div>
    </div>
